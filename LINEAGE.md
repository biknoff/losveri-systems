...
- **HaditFugue (Windows era, retired).** The NT8 lineage's last stand: a Python/Nautilus orchestrator bridged through C# into NinjaTrader, running a supervised runner with multi-account routing and live tick recording on a dedicated Windows box. At its verified peak it ran **11 real prop-firm evaluation/funded accounts (10 concurrent in a single day) for the two operators, with 978 live executions counted in one three-week window alone** — recomputed in 2026 from a preserved snapshot of the platform's own database (which keeps only a rolling window, so lifetime volume is larger than what survives). Retired; the box no longer exists.
