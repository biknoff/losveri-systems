...
- **Speaker identification** is a separate, working pipeline: `brain/speaker_id.py` loads an ECAPA-TDNN model (`speechbrain/spkrec-ecapa-voxceleb`) and compares embeddings against `enrollment/embeddings.json`, built from two real enrolled voice samples (`leo.wav`, `mariele.wav`). Real session logs (`logs/sessions/*.jsonl`) show this running end-to-end: timestamped, per-segment records carrying `speaker`, `speaker_confidence`, and `language` fields (message text withheld here — see EVIDENCE for the redaction note).
