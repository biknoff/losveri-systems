...
**HADIT** is the deployment layer. NUIT found ~20 strategies trading different hours and weekdays, so HADIT became a scheduler-orchestrator: regime awareness turns strategies on and off, sizing flexes with margin. Its lineage runs NT8 + C# DLL on a Windows box (Chicago) → Python/Nautilus → the **Rust engine** running today, watched over by the family's own **cockpit** — a real web application in its own right (~34k lines, dozens of independent data feeds, a live chart, a regime banner) that renders what the engine and the watchers already decided. A read-only mirror of that same cockpit is what the public sees at glass.i-ii.trade — one write-free window onto the real operational surface, not a separate reimplementation. And it grew outward: the cockpit's own manual-trading panel became a service (**Miami** — connect a broker, run multiple accounts, copy-trade; external users trade real money on it today), and watching those users chart in one app, screenshot into another, and trade in a third produced the product: **i-ii.trade** — one place to trade, draw, drop voice notes on the chart, share with friends ("together"), or run your own strategy scheduler ("alone"), the same orchestration the family runs live.
